...
- coding runtime 里 **原生快照是主、harness JSONL 是从**：resume / undo / rewind / restore / 会话目录 / 租约 / 落盘失败 fail-close 只认 `SessionManager`。`session-persistence-jsonl` 这一行继续写，但 `resume = false`；本产品在 `CODING_DEFAULTS` 里把它换成 coding 自己的 `session-journal` 行（`coding/src/session_journal.rs`，harness 原行不改），写在 `<home>/sessions/harness/<bucket>/`（目录名 `SessionManager::JOURNAL_DIR`，catalog 扫描跳过它）——它和原生 transcript 不共用一个文件，任何一方都不得读对方的文件当权威。历史上写进原生桶里的 journal 文件由 `SessionManager::is_journal_file` 识别，recall / catalog 跳过、`session-journal` 仍能原地读写。
  - **方向已变（2026-09-17，`docs/adr/0024-the-session-log-is-the-authority.md`）**：会话的唯一权威改为 harness 事件日志，lead / team 成员 / task 子 agent 都落盘；原生快照不再是任何东西的权威。本条与第 39 行描述的是**迁移落地前的现状**：改会话持久化相关代码前先读 0024，不要再按「原生为主」加新依赖；迁移步骤逐项落地时同步改这两条。
