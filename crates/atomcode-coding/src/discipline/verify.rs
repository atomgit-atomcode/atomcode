...
const NUDGE: &str = "You made code edits but have not verified them. Run a fast check (`cargo check`, `tsc --noEmit`, or the equivalent for this project) to catch errors before finishing. Do NOT start a long-running process (dev server, watcher, full build).";
...
struct State {
    ...
    /// Number of conversation messages at the start of the current turn. Only edits
    /// after this index are considered for the verification nudge --- edits from
    /// previous turns are ignored, preventing cross-task false positives.
    turn_start_msgs_len: usize,
}
...
/// Scan the conversation starting from `start_idx`: returns the tool_call_id of the
/// most recent successful edit AFTER `start_idx` IF it has no successful `bash` after
/// it (i.e. unverified), else `None`.
fn unverified_edit_id(convo: &Conversation, start_idx: usize) -> Option<String> {
    ...
    for msg in &convo.messages[start_idx..] {
        match msg.role {
            Role::Assistant => {
...
impl LifecycleHooks for VerifyCadenceHook {
    ...
    async fn session_start(&self, _convo: &mut Conversation, resumed: bool) {
        if !resumed {
            ...
            state.nudged_for = None;
            state.turn_start_msgs_len = 0;
        }
    }

    /// Record the message count at the start of each turn so `offer_continuation`
    /// only scans messages from this turn onward --- preventing cross-task false
    /// positives where edits from a previous turn trigger an irrelevant nudge.
    async fn turn_start(&self, convo: &mut Conversation) {
        let mut state = self.state.lock().unwrap_or_else(|e| e.into_inner());
        state.turn_start_msgs_len = convo.messages.len();
        // Reset nudged_for on turn boundary: the previous turn's nudge state
        // should not suppress a fresh nudge in the new turn.
        state.nudged_for = None;
    }

    async fn offer_continuation(&self, convo: &Conversation) -> Option<String> {
        let start_idx = { self.state.lock().unwrap_or_else(|e| e.into_inner()).turn_start_msgs_len };
        let edit_id = unverified_edit_id(convo, start_idx)?;
        let mut state = self.state.lock().unwrap_or_else(|e| e.into_inner());
...
mod tests {
    ...
    async fn nudge_of(msgs: Vec<Message>) -> (VerifyCadenceHook, Option<String>) {
        ...
        let start_idx = convo.messages.len();
        convo.messages = msgs;
    ...
    #[tokio::test]
    async fn previous_turn_edits_do_not_nudge_new_turn() {
        // Simulate: Turn 1 has an unverified edit, Turn 2 has no edits.
        // The nudge should NOT fire for Turn 1's edits.
        let hook = VerifyCadenceHook::new();
        let mut convo = Conversation::new();

        // Turn 1: model edits code
        convo.messages.push(assistant_call("e1", "edit_file"));
        convo.messages.push(tool_result("e1", false));

        // Turn 2 begins: record turn start
        hook.turn_start(&mut convo).await;

        // Turn 2: model only reads, no edits
        convo.messages.push(assistant_call("r1", "read_file"));
        convo.messages.push(tool_result("r1", false));

        // Should NOT nudge --- Turn 2 has no edits, and Turn 1's edits are before the boundary
        assert!(hook.offer_continuation(&convo).await.is_none(),
            "must not nudge for edits from previous turn");
    }

    #[tokio::test]
    async fn edits_in_new_turn_after_boundary_do_nudge() {
        // Simulate: Turn 1 has edits, Turn 2 has fresh edits ---> should nudge.
        let hook = VerifyCadenceHook::new();
        let mut convo = Conversation::new();

        // Turn 1: model edits and verifies
        convo.messages.push(assistant_call("e1", "edit_file"));
        convo.messages.push(tool_result("e1", false));
        convo.messages.push(assistant_call("b1", "bash"));
        convo.messages.push(tool_result("b1", false));

        // Turn 2 begins
        hook.turn_start(&mut convo).await;

        // Turn 2: model edits again without verifying
        convo.messages.push(assistant_call("e2", "edit_file"));
        convo.messages.push(tool_result("e2", false));

        // SHOULD nudge --- Turn 2 has unverified edits
        assert!(hook.offer_continuation(&convo).await.is_some(),
            "must nudge for unverified edits in the current turn");
    }
}
