...
impl Moment {
    ...
    pub fn cancel_idle(&mut self) -> bool {
        ...
        self.notice = Some(
            Notice::for_ms("再按 Ctrl+C 退出", false, self.now, QUIT_HINT_MS).below(),
        );
        false
    }
}
...
mod tests {
    ...
    fn work_between_the_two_presses_cancels_the_pending_exit() {
        ...
        assert!(!m.cancel_idle(), "real work between presses disarms the exit");
    }
    ...
    fn disarming_leaves_a_copy_hint_alone() {
        ...
        m.disarm_quit();
        assert!(m.notice.is_some(), "the copy hint is not the latch's to clear");
    }
