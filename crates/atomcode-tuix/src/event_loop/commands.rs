...
fn spawn_runtime(
    ...
    // Spawn through the injected engine spawner so in-TUI session switches run on
    // the new stack too. It reads the CURRENT config/working_dir, keeping /model
    // /provider /cd honoured.
