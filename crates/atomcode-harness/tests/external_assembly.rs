...
async fn a_product_can_keep_its_users_state_out_of_the_atomcode_home() {
    ...
    // The other door onto the same memory: `/remember`, which a person runs.
    //
    // It goes through the same tool, and it has to be handed the same file — a
    // command that fell back to `MemoryStore::global()` would write to
    // `$ATOMCODE_HOME` while the model's tool wrote where the row said, and the
    // product would have its users' memory in two places without a word.
    let agent = app
        .context()
        .service::<atomcode_harness::seams::AgentsSvc>()
        .expect("the agent registry")
        .list()
        .into_iter()
        .next()
        .expect("one agent");
    //
    // `forget` rather than `remember`, because `remember` without a scope writes
    // the project tier — it is `forget` that reaches across every tier, global
    // included, which is exactly the reach that has to land in the right file.
    let forget = app
        .context()
        .service::<atomcode_harness::seams::CommandsSvc>()
        .expect("the command catalog")
        .find("forget", &agent)
        .expect("the `forget` command the memory row registers");
    forget
        .run(agent, "m9q3")
        .await
        .expect("the command must go through");
    assert!(
        !MemoryStore::new(global.clone())
            .load()
            .iter()
            .any(|e| e.contains("m9q3")),
        "`/forget` did not reach the global tier the row named"
    );
    assert!(
        MemoryStore::new(global.clone())
            .load()
            .iter()
            .any(|e| e.contains("m9q2")),
        "`/forget` took more than the line it was given"
    );
