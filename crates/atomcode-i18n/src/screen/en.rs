...
pub(super) fn en(msg: Msg<'_>) -> Cow<'static, str> {
    match msg {
        ...
        Msg::CmdAboutRewind => "go back to before a turn: the conversation, the workspace or both; pick one from a list with no argument".into(),
        ...
        Msg::RewindPointAbout { turn, files } => format!("turn {turn} · {files} files changed").into(),
        Msg::RewindPickerHint => "back to before which turn · enter goes there".into(),
        Msg::RewindNoPoints => "there is no turn to go back to yet".into(),
        ...
        Msg::OnboardIntroLine2 => "three steps: pick the language, sign in, and look at what came of it.".into(),
