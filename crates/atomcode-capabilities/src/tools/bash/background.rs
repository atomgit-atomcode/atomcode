...
/// Accumulated output as a bounded tail plus the ABSOLUTE count ever appended, so a poll
/// cursor keyed on the absolute offset survives front-drops.
struct Buf {
    tail: Vec<u8>,
    total: usize,
}
...
impl Buf {
    ...
    /// Text appended since absolute offset `delivered`; returns `(text, new_delivered,
    /// truncated)` where `truncated` means some undelivered bytes were dropped by the cap.
    fn since(&self, delivered: usize) -> (String, usize, bool) {
        if delivered >= self.total {
            return (String::new(), self.total, false);
        }
        let retained_start = self.total - self.tail.len();
        let (slice, truncated) = if delivered >= retained_start {
            (&self.tail[delivered - retained_start..], false)
        } else {
            (&self.tail[..], true)
        };
        (String::from_utf8_lossy(slice).into_owned(), self.total, truncated)
    }
}
...
/// The result of a `poll`: incremental text plus the job's status at read time.
...
/// Return output appended since the last poll plus the current status. A FINISHED job is
/// reaped from the store on this call (its output is fully delivered here), so a later poll
/// of the same id reports "no such job".
pub(crate) fn poll(job_id: &str) -> Result<PollResult, String> {
    ...
    let (text, new_delivered, truncated) = job.shared.output.lock().unwrap().since(job.delivered);
    job.delivered = new_delivered;
    let status = job.shared.status.lock().unwrap().clone();
    if !matches!(status, Status::Running) {
        // Terminal status is only set AFTER both streams hit EOF, so everything is
        // delivered by the `since` above — safe to drop the entry now.
        guard.remove(job_id);
    }
    Ok(PollResult {
        text,
...
/// Await the next line of an optional reader; parks forever when the stream is absent so a
/// disabled `select!` arm (guarded by `*_done`) never resolves spuriously.
...
async fn reader_task_unix(
    ...
    let mut out: Option<Lines<BufReader<tokio::process::ChildStdout>>> =
        stdout.map(|s| BufReader::new(s).lines());
    let mut errs: Option<Lines<BufReader<tokio::process::ChildStderr>>> =
        stderr.map(|s| BufReader::new(s).lines());
    ...
    while !(out_done && err_done && exited) {
        tokio::select! {
            biased;
            ...
            line = next_line(&mut out), if !out_done => match line {
                Some(l) => shared.output.lock().unwrap().append_line(&l),
                None => out_done = true,
            },
            line = next_line(&mut errs), if !err_done => match line {
                Some(l) => shared.output.lock().unwrap().append_line(&l),
                None => err_done = true,
            },
            status = reaper.wait_and_disarm(), if !exited => {
                exited = true;
...
mod tests {
    ...
    /// Poll at 20ms until terminal or a ~1s budget, accumulating every chunk.
    ...
    fn buf_since_delivers_increments_and_flags_truncation() {
        ...
        b.append_line("one");
        let (text, delivered, truncated) = b.since(0);
        assert_eq!(text, "one\n");
        assert_eq!(delivered, 4);
        assert!(!truncated);
        // Nothing new since the cursor.
        let (text, _, _) = b.since(delivered);
        assert_eq!(text, "");
    }
}
