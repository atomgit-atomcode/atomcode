...
/// `$ATOMCODE_HOME/memory.md` + per-project `<root>/.atomcode/memory.md` + the
/// machine-local `<root>/.atomcode/local/memory.md` — the SAME files production
/// reads/writes, so the two stacks share one memory) + a
