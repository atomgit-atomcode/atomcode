...
pub struct AgentLoop {
    ...
    retry_count: usize,
    /// One-shot flag: have we already retried this user-turn after an
    /// empty-response from the provider? Empty responses split into two
    /// causes the framework can't tell apart from inside the runner —
    /// (a) AtomGit / litellm-style soft rate limit (200 + empty SSE
    /// after per-minute output quota exhausted; 5/13 21:15 atomgr
    /// session Turn 12 returned in 4.3 s with text/reasoning/tool_calls
    /// all zero, right after Turn 11 emitted 1822 completion tokens),
    /// (b) deterministic model EOS (model consumed budget on reasoning
    /// and emitted EOS with nothing). One 15-s backoff retry recovers
    /// case (a) for free; case (b) re-fails fast and bails. Reset at
    /// the start of each new user message so a transient quota hit on
    /// turn N doesn't poison turn N+1.
            ...
            match result {
                ...
                TurnResult::Failed(e) => {
                    ...
                    if (is_messages_illegal || is_context_overflow) && self.retry_count < 2 {
                        ...
                        let _ = self.event_tx.send(AgentEvent::TextDelta(msg));
                        continue;
                    } else if e.contains(crate::provider::NON_RETRYABLE_MARKER) {
                        // Hoisted above `is_rate_limited`: NON_RETRYABLE is a
                        // strong signal that no amount of retrying changes the
                        // outcome (same context + same prompt + same model =
                        // same failure). The marker is generated by paths that
                        // KNOW the failure is deterministic — provider stream
                        // caps (SSE byte / max_tokens), validator-rejected
                        // tool calls, repeated empty responses. When such an
                        // error also happens to contain rate-limit-y substrings
                        // (e.g. an SSE cap error that mentions "regenerate"
                        // near "rate-limited", or the upstream proxy mixing
                        // 429 chunks into a non-recoverable stream), the OLD
                        // chain order ran the rate-limit retry path FIRST and
                        // burned 3+6+9+12+15=45 s before falling through here.
                        // 5/14 00:31 atomgr Turn 13-17: 4 rate-limit retries
                        // followed by SSE cap [non-retryable] — the marker was
                        // there all along, just routed around.
                        //
                        // Order with respect to context_overflow (above): kept
                        // BELOW. context_overflow has a compact-and-retry
                        // recovery semantics that's qualitatively different
                        // from "same context retry"; if a future error string
                        // somehow tagged both NON_RETRYABLE and overflow, we
                        // still want the compact path to try once.
                        ...
                        continue;
                    } else if is_empty_response_error(&e) && !self.empty_response_retried {
                        // Soft rate-limit recovery path. 5/13 21:15 atomgr
                        // session Turn 12 returned a 200 with text=""
                        // reasoning_content="" tool_calls=[] in 4.3 seconds,
                        // right after Turn 11 emitted 1822 completion tokens.
                        // The 4.3 s latency is the smoking gun: too fast for
                        // model inference, consistent with a litellm proxy
                        // returning an empty SSE body when the per-minute
                        // output quota is exhausted. Provider doesn't surface
                        // a 429 (would route through `is_rate_limited` above),
                        // so we detect the SHAPE of the failure instead.
                        //
                        // 15 s wait is calibrated against the AtomGit quota
                        // window we've observed; long enough that a
                        // per-minute reset has typically fired, short enough
                        // that a truly deterministic empty (model burned its
                        // output budget on reasoning and emitted EOS) bails
                        // in ~20 s total (one fast empty + 15 s + one fast
                        // empty) without the historical 4-5 min waste.
                        //
                        // ONE retry only (gated by `empty_response_retried`,
                        // reset per user message in `process_user_input`).
                        // Repeated empties within the same user turn mean
                        // the cause is NOT transient — fall through to the
                        // bail-out path on the next iteration.
                        self.empty_response_retried = true;
                        let _ = self.event_tx.send(AgentEvent::TextDelta(
                            "\n[Empty response — likely soft rate-limit, retrying in 15s...]\n"
                                .to_string(),
                        ));
                        tokio::time::sleep(Duration::from_secs(15)).await;
                        continue;
                    } else if is_empty_response_error(&e) {
                        // Second empty in the same user turn — not transient.
                        // Surface as non-retryable so the user sees the
                        // failure immediately and can decide whether to
                        // change strategy (rephrase, switch model, /clear).
                        self.datalog.log_error(&e);
                        let _ = self
                            .event_tx
                            .send(AgentEvent::Error(public_error_message(&e)));
...
/// Empty-response failure: the runner reached the end of the stream with
/// `text_buf` empty, `tool_calls_buf` empty, and `reasoning_buf` empty
/// (or already promoted into `text_buf` and still nothing). Matches the
/// signature string `turn/runner.rs` emits at the empty branch.
///
/// Used by the retry chain to route empty responses through ONE 15-s
/// backoff before bailing — covers the AtomGit / litellm "200 + empty
/// SSE" soft rate-limit case observed 5/13 21:15 atomgr Turn 12 (4.3 s
/// response time, all three response fields zero, occurring right after
/// a 1822-token output burst). Provider doesn't surface a 429 in this
/// case, so `is_rate_limited_error` can't catch it.
...
mod classifier_tests {
    use super::{
        ...
        is_empty_response_error, is_rate_limited_error, public_error_message, public_error_reason,
    ...
    /// The exact error string emitted by `turn/runner.rs` at the empty-response
    /// branch must match `is_empty_response_error` — that's the contract the
    /// retry chain relies on to route AtomGit / litellm soft-rate-limit
    /// failures through the dedicated 15-s-backoff branch instead of bailing
    /// out immediately or burning the catch-all 3 / 6 / 9 s retry budget.
    /// If a future runner refactor renames the message, this test catches the
    /// drift before the user hits "empty 200 in 4.3 s → permanent bail" again.
    #[test]
    fn empty_response_error_is_detected_with_runner_emitted_text() {
        // Lifted verbatim from `turn/runner.rs` empty-response branch.
        let runner_msg = "Provider returned an empty response (no text, no tool calls). \
        ...
        assert!(is_empty_response_error(runner_msg));
    }

    /// Negative case: unrelated provider errors must NOT route through the
    /// empty-response branch. Otherwise rate-limit, auth, context-overflow,
    /// and stream-cut failures would all incorrectly trigger a 15-s backoff
    /// before falling through to their real handlers — a slower, weirder UX.
    #[test]
    fn empty_response_classifier_does_not_match_other_errors() {
        assert!(!is_empty_response_error("API error (429 Too Many Requests)"));
        assert!(!is_empty_response_error("Stream timeout: no data received for 600 seconds"));
        assert!(!is_empty_response_error(
            "[non-retryable] Response exceeded max output tokens mid tool_call."
        ));
        assert!(!is_empty_response_error(""));
    }
