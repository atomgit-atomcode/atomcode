...
mod tests {
    ...
    /// Regression (migration-trap fix): the shape `enabled=true + auto_detect=true +
    /// delay=150 + no servers` is a VALID intentional opt-in — the exact config a user
    /// writes to turn LSP on with the built-in servers. Loading it must PRESERVE `enabled`.
    /// The old `migrate_legacy_lsp_default` heuristic silently wiped this shape (it was
    /// byte-identical to the pre-2026-05-07 auto-written default), so a user could not turn
    /// LSP on the obvious way. v2's LSP is lazy + degrades gracefully, so the migration's
    /// premise ("old installs spawn rust-analyzer every startup") no longer holds.
    #[test]
    fn load_preserves_legacy_shape_lsp_opt_in() {
        let toml_str = r#"
        ...
        let cfg = Config::load(tmp.path()).expect("config loads");
        assert!(cfg.lsp.enabled, "explicit [lsp] enabled=true must be honored, not reset");
        assert!(cfg.lsp.auto_detect, "explicit auto_detect=true must be honored");
    }
