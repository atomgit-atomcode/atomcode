...
> **读之前先知道范围(2026-09-12 更新)**:这份文档论证的架构已经实现在
> `atomcode-tui` 里,而那个 crate 现在是**产品 UI 的去处**,要替换掉
> `atomcode-tuix`。见 [`adr/0012`](./adr/0012-atomcode-tui-replaces-tuix.md);
> 之前「只做无头前端」的定位在 [`adr/0011`](./adr/0011-atomcode-tui-is-the-headless-front-end.md),已被取代。
>
> 下面的论证仍然成立,而且正是它让「不用 tty 跑完整 UI 端到端」成为可能——
> 替换路线上每个新行落地当天就能测,靠的就是这个。

**状态:已实现(定位见 adr/0012)。** 这份文档论证一件事——把 TUI 拆成插件之后,
...
7. **退出时就地给出记录。**
   (决定:`plugin.rs` 退出路径只 `restore()` 终端,不再回吐 transcript)
   全屏按终端语义在离开备用屏幕那一刻整块丢弃,退出后屏幕上看不见刚才
   发生了什么——这是全屏接受的代价,见下方 §九 的取舍记录。
   记录不靠终端滚动条兜:会话本身是事件日志(`docs/adr/0024`),
   `--continue` / recall 读回同一条记录,比往正常缓冲区里重打一份纯文本更完整
   (保留颜色、折叠、面板),也不受终端宽度与滚动条上限影响。
   曾经有一条「退出时把整条流回吐到正常缓冲区」的义务,现予取消:它是一份
   与日志不同源的副本,只能事后补救,且回吐的字节里带着工具给的转义序列
   (`ESC[2J` 会清掉它正要交还的那段滚动条),留在终端里没有重绘 diff 兜底。
...
6. **退出后屏幕上不留内容。** 取消「退出时回吐整条流」(§四 义务 7)之后的代价:
   终端在离开备用屏幕那一刻把整块矩形丢掉,退出后你看见的是原始 shell 提示符,
   刚才那一屏不在滚动条里。补偿是记录本身——会话是事件日志(`docs/adr/0024`),
   `--continue` / recall 读同一份,比重打的纯文本更完整。但「滚上去看一眼」
   这个动作确实没有了,它对开发者工具是一条真实痛感;若实测中痛感超过了
   分屏与折叠的收益,ADR 0006 的失效条件已经写明该重估这个形态。
