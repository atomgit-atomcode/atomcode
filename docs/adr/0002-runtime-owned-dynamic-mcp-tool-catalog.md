...
- **已实现（2026-09-24）**：CodingRuntime 持有一个 `McpConnectionPool`（`capabilities::mcp::pool` 只提供机制），经 `PrepareOptions::mcp_pool` 交给每次 prepare。同目录切会话（新建 / 恢复）时，新 generation 的 registry 接手池中「连接标识逐字段相等且仍连着」的 server，连同其上次列出的工具（定义逐字节不变，请求缓存前缀不动），不再重启；每个 generation 的 registry 仍各自独立（授权、别名、取消、事件不共享）。池只收「在用 registry」（池的 owner）的连接：启动、提交、候选失败回退时由在用 registry settle 成为 owner 并只持有它自己的连接；候选、被替换的、prepare 半途失败留下的 registry 晚到的连接一律拒收，随各自 registry 关闭。`withdraw_mcp_tools`（reload、untrust、logout）清空池并置 owner 为空，被撤回的 registry 不再成为 owner。第一条后果里「刚切换的首个 turn 可能看不到 MCP 工具」对同目录切会话不再成立。设计与判据见 `docs/plans/2026-09-24-mcp-connection-pool-across-sessions-design.md`。
