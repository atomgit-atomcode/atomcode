...
- **成员面板是切换入口**(`tui/modules/team.rs`,`tui-panel-team`):竖排列表,第一行是「主」(lead),
  其后是**仍在跑的**成员;可键盘导航、可鼠标点击。选中一行即切换,**整个输入框与流都换成那个 agent 的**——
  输入发给它(第 4 节),流、状态栏、任务清单、steering、提问面板都显示它的。选「主」切回 lead。
  面板今天的文档写着「刻意不显示成员自己的流」,随本决定改写。
- **面板只画在跑的东西,停掉的成员不是一行**(2026-09-19 修订):一个成员 `stop` 之后不会回来、
  不占下一次委派的名额,留着它的行就是一行既不说什么也做不了什么的装饰。团队没人时整个面板
  不占位(`Hug(0)`,同 `todo` 无计划时),标题行也不画——一块总在那写着「还没有成员」的条子是
  这块面板正是为了去掉的那种 chrome。要看停掉的成员说什么,走 `/agents`(第 5 节)。
- **切过去看的是谁不因成员停下而丢**:正在看某个成员时它被 `stop`,屏幕交回 lead——面板里
  已经没有能切回去的行了,不这么做人就留在一个没有任何出口的会话里。
- **键盘与鼠标**:`Tab` 把焦点移到成员面板(`Tab` / `Shift-Tab` 终端能收到,`tui/surface.rs:1497-1498`,
  键位表里还没绑);焦点在面板时 `↑` / `↓` 移动、`Enter` 切换、`Esc` 或再按 `Tab` 回输入框不切换。
  鼠标悬停高亮、点击直接切换。面板里标出当前看的是谁,状态栏也写上当前 agent 名。可切换的目标
  (`targets`)与画出来的行(`rows`)用同一个判据过滤,否则一次点击会落到它下面那一行上。
- **键盘与指针**是两件事(2026-09-19 修订):面板常驻,而鼠标划过它就只是划过。所以「指着哪一行」
  (`Moment::team_cursor`)跟指针走——悬停点亮那一行,因为那就是「按下会落到谁」;而「键盘是不是
  面板的」(`Moment::team_keyboard`)只由 `Tab` 置位。原先两者是同一个字段,于是鼠标扫过面板就等于
  抢走键盘,人在输入框敲的字全被面板吃掉(面板只认 `↑↓`/`Enter`/`Esc`),要按 `Esc` 才回得来。
  标题里那行键位说明跟键盘走而不是跟高亮:没有键盘时它没有键可报。提问面板不是这个问题的反面
  例子——它的键盘挂在「有疑问待答」这个会话事实上,指针同样无法创建焦点。
...
团队面板不画停掉的成员(第 3 节),所以这个入口是 **`/agents`**:它列出主与这个会话底下
有过的每个成员(含已停的,并注明已停),选一个就把屏幕切过去(`Action::LookAt`,与面板
点击走同一条路)。屏幕那侧的名册(`tui` 的 `Roster`,经 `tui-team-roster` 交给命令)是唯一
一份,因为它要活的也要停的,而 `Moment` 只带面板要画的那部分。
...
- **成员泵的事件没人读**:成员与 `task` 子 agent 的泵只接命令,它产出的事件没有前端在读;`drive` 关掉这条
  事件通道,不让它在成员活着期间一直积压。
- **产品的前端连接与 harness 的泵走同一份实现**(M4.8 时发现产品没接上):寻址信封、目录命令、订阅已经
  不在的成员,harness 的句柄泵与 coding 的 `front_end` 都调 `handle::command_member` /
  `catalog_command` / `Feed::replay_kept`,不各写一份。
- **切过去看已经停掉的成员**:订阅一个注册表里已没有的会话时,从持久化读它的日志推给前端(先描述、再事实,
  之后不再有新事实);只认被委派出去的会话,另一个独立会话不能这样读到。
- **屏幕每个会话一条流**:tui 始终订阅 lead;第一次切到成员时从序号 0 订阅它,之后一直留着,切回来直接画
  已收到的事实,不重新订阅。lead 的回合事件只在屏上是 lead 时移动状态行;屏上是成员时由它的状态移动。
- **面板的可切换行**:`主`,然后注册表公告过的每个成员(停掉的也留着,标已结束,可切过去看)。这块屏幕打开之前
  就停掉的成员只从 lead 日志里知道,照列但不可切换。
