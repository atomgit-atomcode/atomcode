...
> **2026-09-17 修订([`0021`](./0021-front-end-contracts-split-by-layer.md))**:上一句不再成立。
> SDK 面 = 句柄协议(会话内)+ 宿主控制契约(会话切换、撤销、rewind、恢复快照、模型、
> MCP、重载)。undo / rewind / 恢复快照属于后者,进 SDK 面。「只定义一次、各前端是
> 投影」这条不变。原文保留,是为了看得出改过什么。
