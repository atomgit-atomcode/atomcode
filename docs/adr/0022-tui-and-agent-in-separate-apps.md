...
**重载的边界(2026-09-18 M5.1 补充)。** 第二行的「重载走 control patch」落地时分成两段,
按「有没有东西要重连、配置动没动」判:

| 重载时 | 走哪条 | 为什么 |
|---|---|---|
| 配置没动、没有 MCP 服务器、回合之间 | **原地重读**:磁盘上的技能读进活着的注册表,目录片段按同一个 id 重贡献 | 每个持有者拿的都是同一个 `Arc`——`use_skill`/`list_skills`、`skills` 缝、斜杠菜单;重挂会把整棵树连同 MCP 连接一起带走 |
| 有 MCP 服务器 | 重建 | 重连正是重建在做的事;把连接搬进新图不是一次 patch |
| 配置动了 | 重建,用新配置 | 权限规则、hooks、工具全由配置决定,逐行 patch 不是这个宿主敢声称的事 |

配置「动没动」由 `HostConfig::fingerprint` 回答(默认 `None` = 说不出来 ⇒ 重建)。
产品实现是配置文件字节的哈希,外加启动时那些不在文件里、却决定模型 id 解析的开关。

**仍未做**:MCP 原地重连(连接建在 `prepare` 里,搬进活着的树需要发布路径也能换),
以及配置改动的逐行 patch。两者都不挡替换路线——重建仍然满足上面的不变式,因为日志才是
权威(0024),重建后的 App 读的是同一份日志、序号接着编。
...
**解禁(2026-09-18,M5 落地后)。** 上面那份「不开放」清单已经全部解禁,今天的 tui 就是这样
发的(`/undo`、`/rewind`、`/reload`、`/login`、`/logout`)。解禁条件是第 2 节表的第二、三行
各自落地:

- 第三行「同会话,内容变了」——撤销与恢复做成日志里的事实、改投影不改历史(M5.3);
- 第二行「同会话,内容不变」——重载在能原地重读时不重建、配置动了才重建(M5.1,
  边界见本节上方的「重载的边界」);登出/登录只换模型行、不拆 agent(M5.2)。

**这一节到此为止只剩历史价值**:它记的是分期本身,不是一份仍在生效的禁令。谁再读到,
不要照着往回删——要改哪一项能不能开放,改的是第 2 节的表,不是这里。
