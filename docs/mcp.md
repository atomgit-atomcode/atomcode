...
> **切会话不重连**：同一个 runtime 里在同一目录新建或恢复会话时，已经连上的 server 直接交给新会话继续用（进程不重启、状态不丢），工具定义与上一会话逐字节相同。配置里某个 server 的启动参数、URL、环境变量展开值等变了、它已经断开、或切到了别的目录，才会为它重新连接。要强制全部重连，用 `/mcp reload`；信任、登录状态变化同样会全部重连。
