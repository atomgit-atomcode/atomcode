...
- [x] **G1 `/usage` 画图**（2026-09-20）。画在设置面板的「用量」页签里，不再另开命令。
      ...
      **第四轮：三块逐条核过 tuix 三个页签，把缺的全补了（2026-09-20）。**
      核的办法是打开 `tuix/modals/usage.rs` 的 `current_rows` / `overview_lines` /
      `models_rows` 逐行读，不是看截图目测。结论：模型那页早就全了，另外两页各缺
      一整块。
      - **订阅块**（`CodingPlan Pro · 生效中` / 领取·到期 / 剩余 N/M 天 + 天数条）
        整块没有。数据是 `codingplan::types::PlanInfo`，**就在 `status_v2()` 同一个
        响应里**——和 `calls_used`、`model_tokens` 是同一个取数点的第三、第四次同样
        丢法。补进契约 `Entitlement`，daemon 那侧提成 `plan_from()` 配判据。
        取数走新的 `fetch_plan()` 而不是给 `fetch_windows` 加字段：后者在 429 热路径
        上、答案会被缓存和老化，订阅数据没有理由进那条路。代价是开面板时多问一次
        `status_v2`——面板是手开的，429 不是。
      - **日历热力图**（月份表头 + Sun–Sat + 六级色阶 + 少/多图例）整块没有。
        只要 `daily` 就能画，不用扩契约。跨度外的天留白而不是补成矩形：补满等于
        宣称四天没给过的数据，还会把后面每一天都放到错的星期几上。
      - **总览七项**原来只有三项（总 token / 请求次数 / 活跃天数），缺最常用模型、
        最活跃日期、最长连续天数、当前连续天数；而且是挤在一行的句子，不是对齐的
        两列。对齐必须按**显示宽度**——`请求次数` 四个字符八格、`总 Token 数` 九个
        字符十一格，按字符数补齐会让每个值落在不同的列（对面报过这个 bug，继承
        修法比重新发现便宜）。
      - 另外三处小的：条用完转红（`Role::Error`）、额度百分比、重置倒计时改成
        `00:00:11` 的秒级（人盯着等窗口回来时看的就是这个数，四舍五入到小时会让
        最后一分钟看起来像第一分钟）。
      热力色阶走新的 `Role::Heat(n)`，**算出来的不是挑出来的**：从面板底色往 accent
      混。tuix 写死 `[231, 217, 210, 174, 131, 88]`（白→深红），在浅色终端上是渐变，
      在深色终端上是一排亮块褪成看不见。它也不能套文字对比度的门——最空那一级的
      职责就是贴着底色——所以 `floor` 给 0，另立判据「每一级必须比下一级更重，且
      在任何底色上不改方向」。
      **第五轮：这一页现在比面板高，得能滚（2026-09-20）。** 用户截图：框线没了、
      尾巴被切。根因是 `fit()` —— 它给列表腾地方的办法是**先删 `Row::Rule` 和
      `Row::Blank`**，再 `truncate`。对一个只超一两行的列表这是对的；对一个超二十行
      的页面，它先吃掉面板自己的上下两条边，再把剩下的截断，于是屏上是一个**没有
      下边、而且说停就停**的页面。
      改法：非设置页走新的 `window()`，头两行（页签 + 横线）和收口那条横线**永远
      不让**，让位的是正文，而且让位时拿一行出来说「下面还有 N 行 ↓」——悄悄只显示
      三分之二的页面是一种沉默的谎。`Panel` 加 `scroll`，上下/翻页键在非设置页滚动
      （设置页仍是移光标：一个既能筛又能滚的列表有两种办法弄丢你正看着的那行），
      换页归零。偏移在**数行数的那一侧**夹住，不在按键那侧——按键那侧不知道页面
      多长，而滚过头的结果是一个空面板，偏偏干这事的键是人按住不放的那个。
      判据 2 条、证伪 5 种。写的时候又踩了一次「判据抓错行」：断言「表格在折叠线
      以下」，而那个模型名同时出现在总览的「最常用模型」里，于是加了第二个模型、
      只断言那个只在表尾出现的。
