...
- [x] **G1 `/usage` 画图**（2026-09-20）。画在设置面板的「用量」页签里，不再另开命令。
      ...
      **第五轮：这一页现在比面板高，得能滚（2026-09-20）。** 用户截图：框线没了、
      ...
      只断言那个只在表尾出现的。
      **第六轮：拆成两个页签，滚轮接上（2026-09-20）。** 用户要「总览和模型用量
      放到 Stats 里」，并给了一张带二级页签（Overview / Models）的图。
      拆的依据不是「太长了放不下」，而是两页答的是两个问题：**Usage 答「我还能做
      多少」**（额度窗口 + 背后的订阅），**Stats 答「我做了多少」**（日历 + 七项 +
      图 + 表）——后者是读的，不是拿来操作的。
      二级页签用 `Panel::stats`（`StatsPage`），放在 panel 上而不是页面自己的状态里：
      人正读模型表、去 Config 看一眼再回来，指的是回到模型表。
      **左右键走眼睛所在的那一行**——有二级页签就是二级，没有就是主页签；`Tab` /
      `Shift-Tab` 永远是主页签，所以外层那行不会变得够不着。
      总览的七项改成两栏（照给的图）：左栏讲量、右栏讲时间。对齐仍按显示宽度，而且
      右栏起点是**离左栏固定距离**而不是「最长的左值再加二」——后者会让数字多一位时
      整列平移，人每次刷新都要重新找。
      滚轮：`Host::settings_wheel(x, y, by)`，**只在指针真的落在面板上时**才接管。
      开着面板就吞掉所有滚轮事件，等于把滚轮从它根本没遮住的那片对话上抢走。设置页
      不滚（它是打字筛的）。
      顺带清了两处：`elsewhere()` 改成问 `draws_itself()`（原来判据和它各存一份
      「哪些页是占位」的名单，页面做完后就会对不上）；单栏的 `UsageLine::Pair` 与
      `pairs()` 成了死码，删掉。
      判据 3 条、证伪 6 种；另有 6 条既有判据改指到新的页（辅助函数收成
      `page_on` 一个，三个名字，这样「某节搬到别页」是一个词的改动）。
      **第七轮：上一轮把 Stats 做成了死胡同（2026-09-20）。** 我让左右键在有二级
      页签的页上去切二级页签——而人正是用左右键沿着主页签走的，于是走到 Stats 就
      只能在它那两页之间打转，**而且屏上没有一句话说哪个键能出去**。
      改回：**左右键永远是主页签**（`Tab` / `Shift-Tab` 同义）；**上下键走页面放在
      箭头底下的那一行**——设置页是光标、有二级页签的页是二级页签、都没有就是滚动；
      翻页键和滚轮永远滚动。判据 `no_page_is_a_dead_end` 对**每一页**断言「左右各按
      一次都能离开」，不是只测出事的那一页。
      同时给所有只读页加了键位提示行（`←→ 换页 · ↑↓ … · esc 关闭`）——死胡同本身
      是逻辑错，但「屏上没说出路」是它能一直卡着人的原因。
      二级页签支持点击：`pages_parts()` 一份数据同时供绘制和命中（同 `header_parts`
      的理由），行号从**实际布局**里读（`Geometry::pages_row`）而不是写死。
      二级页签行和提示行都归进 `window()` 的头/脚，不参与滚动：一行滚走了的页签，
      是在它所属的那一页上够不着的页签。
      证伪 5 种，其中两种第一次**没判红**，是判据不够锋利：常数 3 恰好就是那行的真
      行号（改成 4 才红），以及没有人断言「滚到底二级页签还在」。两条都补了。
      **第八轮：Status 页落地（2026-09-20）。** 把 Claude Code 的 `/status` 和 tuix
      的 `/status` 两份信息融合：版本 / 会话名 / 会话 id / 目录 / 登录 / 模型 / 订阅 /
      用量 / MCP 计数，再加**配置文件、指令文件、记忆文件**三组带 ✓✗ 的清单。
      **文件清单是这一页存在的理由**：人来这一页多半是因为「我的指令没生效」，而那
      十有八九是「那个文件不在你以为的地方」——所以**没找到的文件也要列出来并说没
      找到**；只列找到的那种清单，答案永远是「一切正常」。
      契约加 `HostCommand::Sources` / `HostReply::Sources`，中立说法是「这个会话是
      从哪些文件配起来的，每个找到没有」；分组名是宿主的词（只有它知道自己的文件
      是什么意思）。cli 那侧提成 `source_groups()` 配判据——它有**两条取数路**
      （指令层用加载器自己的答案，其余问文件系统），所以两条路各钉一次、各证伪
      一次；第一次只钉了一条，把另一条改坏判据照样绿。
      五个问题（Context / WhoAmI / McpStatus / Usage / Sources）并发问，页面晚的是
      最慢那个而不是五个之和。MCP 只报计数、按「要不要处理」排序（连不上 → 等信任
      → 连接中 → 已连接 → 没连），名字留在 `/mcp`。
      **顺带删掉占位那条路**：每一页现在都自己画了，`elsewhere()` 塌成永远返回空串，
      `Row::Elsewhere`、`draws_itself()` 和那条不可达分支一起删；两条走遍各页检查占
...
- [~] **G3 这条记错了（2026-09-20 用判据核实）。** 原文写「只在内存里，重启即丢」，
      ...
      **真正还没有的是跨会话**：新开一个会话看不到昨天在别的会话里打过的命令
      ...
      > **2026-09-23 更正**：上面这句把两条路一起记成了"和 0024 拧着"，**只有第二条
      > 拧**。从别的会话日志里把 `UserMessage` 折出来是**读**，不是第二份存储，正是
      > 0024 要的形状（`/worklog` 早就在这么扫日志）。用户当天决定连跨会话一起做，
      > 设计落在 [`2026-09-23-tuix-parity-remaining.md`](2026-09-23-tuix-parity-remaining.md)
      > 的决策 3 与 P2-8。
