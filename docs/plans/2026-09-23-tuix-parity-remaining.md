...
> 启动提示三条在默认屏上全丢（`64f595062`）、插件带的 `commands/` 根本没被
> 加载（`56f05a9ef`）。两条的共同形状是**没有任何一处说它没生效** ——
> 比清单上任何一条「做浅了」都难发现。
...
- [x] **P2-3 `/resume`**（2026-09-24，`534c6ffa4`）。三条里**两条核过是已经有的**：
...
- [x] **P2-5 `/cd`**（2026-09-24，`55edd676e`）。「打字直达」早已有（`.accepting_typed`）；
...
- [x] **P2-6 引导向导**（2026-09-24，`45a395cd2`）。原文：：`Step::Setup` 的三选一里缺「手动配置 provider」那一支
...
- [x] **P2-7 `/diff` 深度**（`faf8c81d0` + `38b12e4b0`）。原文：（依赖决策 1）：文件状态字母、staged/unstaged 分段、
...
- [x] **P2-9 零碎**（2026-09-24）。六条，**做了四条、不做两条**，另外撞出一条清单上
      没有的真回退。
      - [x] **Bash 展示行折 `$HOME`**（`148602f53`）。规矩照 shell 自己的：`~` 只在
        ...
        「谁折谁不折」是悬空的。
      - [x] **`/whoami` 补 auth 文件路径**（`148602f53`）。契约 `Identity` 加
        `stored_at`（路径，永远不是内容）。单起一行排在最后：它答的是**名字答错
        之后**才会问的那个问题。
      - [x] **Orca 的 OSC 11 乱序规避**（`84d462a8b`）。颜色探测拿 DA1 当围栏，
        Orca 1.4.180 在 DA1 之后才回 OSC 11 —— 那条回答于是变成一串看着像打字的
        输入。这一台不问。接线那条只能读源码判（要真 tty，没有测试能驱动的版本）。
      - [x] **`/skills` 链式**（`4f35945f3`）。摸底发现它比这里记的重：**参数是被
      ...
      - [ ] ~~`/rewind` 第二步的「仅代码」档~~ **不做**（确认原决定）。面板砍到两档
        ...
        `/rewind N 代码` 仍然可达。
      - [x] **（清单外）插件带的 `commands/` 根本没被加载**（`56f05a9ef`）。查上面那条
...
- **`/goal` 带图** —— **不做**。目标的提示词是**每一轮重发**的，图跟着重发就是
  每轮一份图的钱，而几乎没有人要的是这个。要把一张图交给模型，普通地贴进
  一个回合就行。
