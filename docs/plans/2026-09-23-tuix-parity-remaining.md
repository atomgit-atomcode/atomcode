...
> **2026-09-25**：暂缓已解除，`/bg` 已在新界面上做出来，设计与判据见
> [`2026-09-25-bg-design.md`](2026-09-25-bg-design.md)。

> **2026-09-24 进展**：P0、1、2 三批**已全部做完**，挂在括号里的几条待定项
> 一次性拍完（新增 **P2.5** 一节）。剩下的只有 **P3（删与收口）**，以及
> P2.5 里那条「值得做、本轮未做」的额度耗尽提示。
>
> 本轮另有**两条清单上没有的**，都是查别的东西时撞出来的，都是真回退：
> 启动提示三条在默认屏上全丢（`e70e11cf1`）、插件带的 `commands/` 根本没被
> 加载（`39d77a409`）。两条的共同形状是**没有任何一处说它没生效** ——
> 比清单上任何一条「做浅了」都难发现。
>
> **2026-09-24 再补一条同形状的**：`/goal <条件>` 与 `/loop <事>` **不跑第一轮**。
> 徽章亮了、状态行在说话，而模型一个字都没收到；第二轮起才有东西——因为每一轮
> 都是由**上一轮结束**推出来的，没有第一轮就没有任何一轮。这一条的「没有一处
> 说它没生效」尤其毒：人接着随便说一句话，那句话就成了第一轮的提示词。
> 修在**运行时**而不是某一个前端里（tuix 原来是自己在驱动侧补一次 submit，
> daemon 的 `/live/goal` 则谁也没补），见下节。
...
## 2026-09-24 第二轮：外部 review 的中低优先级，逐条处理

八条，都已合入 `release/v5.2.0`。列在这里是为了**不被做第二遍**，以及记下
三条「核过之后不做」的理由。

> **提交号是 rebase 之后的。** 这一段第一次写的时候记的是 rebase 前的副本
> （`0db2330e7` / `8d718f9b1` / `df52732d2` / `f5a9b149a`，patch-id 相同但在
> 本分支解析不到）——和 `a558e1804` 刚修过的是同一种错：**先写文档再 rebase**。
> 下次写这种表就在 push 之后再填这一列。

| 条目 | 结果 |
|---|---|
| `/goal` `/loop` 不跑第一轮 | ✅ `23254c212`，修在运行时 |
| App / 浏览器请屏幕跑命令（`register_remote_command_sink` 只有 tuix 接过）| ✅ `e1f547205` |
| 额度问不到说成「这个宿主不计额度」| ✅ `9a87f6338` |
| 剪贴板没法确认也说「已复制」| ✅ 同上 |
| `/diff` 把未跟踪文件说成「没有改动」| ✅ `a32e5ae0c`，并牵出同一函数里第二个 bug |
| `describe_self` 教模型三条不存在的 `/mcp` 子命令 | ✅ `4a07737b8` |
| `/paste <文件>` 没有上限 | ✅ `3a7e175af` |
| `/context prompt` | ✅ `1754ee1b8` |
| 四条启动提示（绕过审批 / root / 离线 / 未信任的插件钩子）| ✅ `5102e09ba` |

**核过之后不做的三条，以及理由：**

- **`/context` 的分项明细**（系统提示词多少 token、工具定义多少、冷区多少）
  —— **不是漏接线，是没有测量点**。`context_stats` 手里只有上一次请求的
  `observed_tokens` 和窗口大小；分项要在组装请求的地方数，是另一件事。
  估一个数画上去，正是这一轮一直在修的那类错——把猜的说成量的。
- **`/mcp trust` / `reload` / `login` 做成命令** —— 面板上都有（一台服务器一
  行，动作就在行上），而 OAuth 登录那条带浏览器、进度与取消，搬成一条命令
  是重做一遍那套流程。改的是**话**，不是落点。
- **`parts.rs` / `runtime.rs` 里 7 处写着 `/mcp reload` 的 Rust 文档注释** ——
  它们不进模型也不进屏幕，这一轮没动；下次碰那两个文件时顺手改。

**这一轮的共同形状**，和前两轮一样：九条里有六条是**屏幕说了自己不知道的事**
（不计额度 / 已复制 / 没有改动 / 三条 `/mcp` 用法），而不是崩溃或报错。它们
共同的判据写法也一样——**钉「两者读起来不一样」，不是钉某一句话对不对**：
只钉后者的话，把两种情况合并回去照样全绿。
...
- [x] **P1-2 CodingPlan 的后台监视**——第 1 个消费者已做(2026-09-23);
      **2、3 于 2026-09-24 拍完**：2 不做、3 值得做且不需要新缝（本轮未做），
      理由与落点见 **P2.5**。
      **额度阈值提示**:`HostCommand::Usage` 多一个 `windows_only`(默认 false,
      老调用方不受影响)——原来那条**一次问三样**(窗口、订阅、已花),各是一次账号
      上的往返,给手开的页面是对的,给定时问的东西就不对:一个只想知道"还剩多少"的
      检查会白白把代价翻三倍。运行时那侧真的只跑 `fetch_windows` 就返回。
      **问的时机绑在回合结束**,不是定时器:回合才是数字会动的时候,空闲时一次都不问;
      再加 30 秒的间隔(`ALLOWANCE_EVERY`,沿用旧前端的数)。时间戳在**发问之前**盖,
      否则间隔内结束的两个回合会在第一个还没回来时各发一次。
      **"什么时候值得说"留在画的那一侧**(`allowance_badge`,纯函数):低于 80% 的
      数字没人会据此改变做法,而一直挂着的那一行,到真要紧时就不再有人看见了。
      ≥95% 转 `Role::Error`,和用量页那根条同两档。
      判据 2(阈值一条、挑哪个窗口一条),各证伪一次。
      **顺带修了一条基线上的红**:`cargo nextest run -p atomcode-host-api` 自
      G2(`/resume` 预览与删除)那次提交起就**编译不过** —— 契约自己的两条穷举
      判据没跟着加 `PreviewSession`/`DeleteSession`/`SessionPreview` 三臂。
      和 H4 是同一种烂法:一批判据跑不起来,而没有人发现。已补。
      **2、3 没做**:模型漂移监视与"额度耗尽推 `/openrouter`"都要打
      `/coding-plan/models`,那是契约里根本没有的一条产品专用缝;它们值不值得
      各开一条契约命令,是单独一件事,不该夹在这条里顺手做。
      原文:
...
- [x] **P1-4 键位补一批**（2026-09-23,分两个提交:行编辑那批,和 `F2`）。
      `F2` 要给 `Key` 加一个 `F(u8)` 变体(这块屏幕此前一个功能键都不认),
      外加一条 `CycleModel` 与它的处理器。**走 `/model <id>`,和面板换模型同一条路**
      ——面板也是发这条命令,所以换模型只有一条路,不是第二条得一直跟它保持一致的路。
      「下一个是谁」提成 `ProvidersView::model_after`(纯函数)单独判:按键本身要
      会话、端口和面板才跑得起来,而这就是决定的全部。**会回绕**(到头停住的循环键,
      人按两下就不按了)、**只有一个模型时什么也不做**(换到已经在用的那个不算换)。
      两处破坏各判红:去掉回绕、去掉 `Shift+F2` 绑定。
...
- [x] **P2-1 `@` 文件补全接 `file_index`**（已做，`plugin.rs:26,540`）。今天是单层 `read_dir` + 纯前缀 + 不读
      `.gitignore`(`tui/src/plugin.rs:504-545`)；对面复用 `capabilities::file_index`
      的跨层子串 + gitignore + 异步索引（webui 的 `@` 也用同一套）。**能力现成**，
      是接线不是重写。
- [x] **P2-2 粘贴健壮性**（`2a6ecf446`）：字符级 paste-burst 聚合（无 bracketed paste 的终端、
      PowerShell 5-12ms 分片）与多条 `Event::Paste` 合并——对面
      `tuix/src/input/reader.rs:39-43,440-546`、`:694-936`（含 IME 假阳性排除）。
- [x] **P2-3 `/resume`**（2026-09-24，`b6ef0a430`）。三条里**两条核过是已经有的**：
      「标出正在跑的那个」在 `modules/resume.rs:276`；「预览带 `provider · model`」是
      **写下来的决定**不是漏做（`cli/src/host.rs:1103-1113`）——provider 在任何地方都不是
      记录下来的事实，`RequestHeader` 只有 model，照抄过去得从模型名去猜一个 provider。
      按 §0.2「新前端更深的不要对齐回去」。真做的是留的那个口子（删会话的端到端判据），
      顺带修出一个真 bug：`/resume` 无参时按**进程启动目录**划范围，传过 `--dir` 或敲过
      `/cd` 之后它列的是别的项目。原文：：预览带上 `provider · model`（对面 `session_picker.rs:734-737`）、
      列表标出「这是你当前正在跑的那个」（`:715-718`）。另：G2 留的那个口子——
      「按两次 Delete 到端口」那段没有端到端判据，给 `tui/tests/e2e.rs` 的 `start_full`
      加第七个参数时一并补。
- [x] **P2-4 塌缩掉的子命令**（`ee43958a7` / `8669af9e4` / `d43ef2331` / `/copy msg` / `/save` 拒覆盖）（一批，都是同一种做法）：
...
- [x] **P2-5 `/cd`**（2026-09-24，`281b022ae`）。「打字直达」早已有（`.accepting_typed`）；
      Tab 补全**落在 `Picker` 上而不是 `/cd` 上**——这条手势对每张列表都成立，而且它是
      纯的（候选就是手里的行，不读盘），不必给分层规矩开口子。原文：：Tab 做 shell 式路径补全、Enter 直接去任意没列出的路径
      （对面 `dir_picker.rs:222-260`、`:128-150`）。这边换来了浏览与书签，**不是纯退**，
      但"我知道路径，让我直接去"这条快路没了。
- [x] **P2-6 引导向导**（2026-09-24，`4b5c9d2c1`）。原文：：`Step::Setup` 的三选一里缺「手动配置 provider」那一支
...
- [x] **P2-7 `/diff` 深度**（`faf8c81d0` + `1b4f020d7`）。原文：（依赖决策 1）：文件状态字母、staged/unstaged 分段、
      详情页 Esc/Left **退回文件列表**而不是整个关掉（今天 `overlay.rs:270-280`
      除滚动键外一律 `Cancelled`，看下一个文件得重敲 `/diff`）。
- [x] **P2-8 跨会话输入历史 + `Ctrl+R` 反向搜索**（`b7fd09a09`）。原文：（决策 3 已定：两个一起做）。
...
- [x] **P2-9 零碎**（2026-09-24）。六条，**做了四条、不做两条**，另外撞出一条清单上
      没有的真回退。
      - [x] **Bash 展示行折 `$HOME`**（`ee0a33599`）。规矩照 shell 自己的：`~` 只在
        一个词的开头才是 home，`:` 和 `,` 不是词边界（`PATH` 冒号表、`host:/path`
        都不展开）。分支也判：折的是 shell 行，文件卡里的路径不折——为此把
        `subject_of` 劈成 `subject_of_within(tool, args, home)`，只钉折叠函数的话
        「谁折谁不折」是悬空的。
      - [x] **`/whoami` 补 auth 文件路径**（`ee0a33599`）。契约 `Identity` 加
        `stored_at`（路径，永远不是内容）。单起一行排在最后：它答的是**名字答错
        之后**才会问的那个问题。
      - [x] **Orca 的 OSC 11 乱序规避**（`078591408`）。颜色探测拿 DA1 当围栏，
        Orca 1.4.180 在 DA1 之后才回 OSC 11 —— 那条回答于是变成一串看着像打字的
        输入。这一台不问。接线那条只能读源码判（要真 tty，没有测试能驱动的版本）。
      - [x] **`/skills` 链式**（`ee4d91b65`）。摸底发现它比这里记的重：**参数是被
        丢掉的**，`/skills review my patch` 列一遍所有 skill 就完事，对打字的人来说
        和「跑起来了」长得一样。现在带名字就跑，一个回合，`---` 隔开。
      - [ ] ~~`/help commands` 子模式~~ **不做**。核过之后这条的前提在新栈里不成立：
        旧前端的 `/help commands` 列的是「人自己写的 markdown 命令」，而那在新栈里
        **就是 skill**（`skills/registry.rs:274-288` 的标准目录里本来就有
        `~/.claude/commands`、`<项目>/.atomcode/commands`），没有第二种命令可以单列。
        旧版多给的那一点信息是「全局还是项目」，而它在任何地方都没留下
        （`Skill` 只有 `source_path`，`source_rank` 把两者都归 0），要补得给
        kernel 的命令缝（`CommandDescription`）加字段——为一个便利改中立内核的契约，
        不划算。`/skills` 已经把它们全列出来了。
      - [ ] ~~`/rewind` 第二步的「仅代码」档~~ **不做**（确认原决定）。面板砍到两档
        是**写下来的决定**并写了理由（`tui/src/rewind.rs:85-89`），而且逃生口还在：
        `/rewind N 代码` 仍然可达。
      - [x] **（清单外）插件带的 `commands/` 根本没被加载**（`39d77a409`）。查上面那条
        时撞出来的真回退：`installed_plugin_skill_dirs` 只走 `skills_dirs()`，从不碰
        `commands_dir()`——而插件是照着上一代前端写的，那边两个目录都读。后果是一个
        全部内容都在 `commands/` 里的插件，装得上、列得出文件数、一条都跑不起来，
        且没有任何一处说它没生效。

### P2.5 — 括号里那几条（2026-09-24 一次性拍完）

> 这几条挂在「要你拍」上很久。一条一条拍完，不做的写清楚为什么不做。

- **`/bg`** —— ~~不在本计划内~~ 2026-09-25 已做（[`2026-09-25-bg-design.md`](2026-09-25-bg-design.md)）。
  不要再当成未决项拿出来问第二遍。

- **P1-2 第 2 个消费者：模型漂移监视** —— **不做**。旧前端每小时探一次
  `/coding-plan/models`，模型没了就提醒。三个理由：
  1. 契约里的 `HostCommand::Models` 答的是**本地目录**（`ModelsSvc`，
     `cli/src/host.rs:1630-1653`），探服务器得新开一条 CodingPlan 专用的缝 ——
     而 `the_contract_speaks_no_products_vocabulary` 那道闸门存在的意义正是挡这个；
  2. 代价是每个跑着的会话每小时一次后台网络请求，换一年里几次的提醒；
  3. 真要紧的那一刻是**掉到地的那个回合**，而那条路上本来就有宿主自己的话。
     要做就把**那句话**改成能说出「现在有的是哪几个」，同样的信息，而且在
     唯一有人会据此行动的时刻。

- **P1-2 第 3 个消费者：额度耗尽推 `/openrouter`** —— **值得做，且不需要新缝，
  本轮未做**。数字已经在手里（`check_allowance`，回合末 + 30s 间隔）。
  **落点推荐：`HostCommand::Readiness`** ——「现在发一个回合会不会被接」正是它问的
  问题，而额度用完就是其中一个答案；它的 `fix` 已经是「宿主点一条命令名、
  屏幕去派发」的现成形状，屏幕侧**一行都不用改**。不要给 `HostReply::Usage`
  另加一个字段：那是第二个答「现在能不能干活」的地方。

- **`/goal` 带图** —— **上一轮记的「不做」理由是错的，已更正**。写的是
  「目标的提示词**每一轮重发**，图跟着重发就是每轮一份图的钱」——**没有这回事**：
  第一轮之后每一轮发的是 `goal_continuation_message(verdict, condition)`
  （`coding/src/controllers.rs:638`），评判者写的一句话，不带图。图只会跟着
  **第一轮**去一次，和普通贴一张图进一个回合完全一样，没有额外的钱。
  **运行时这一侧已经做了**（2026-09-24，见下条）：`DriverCommand::StartGoal`
  带的是 `UserInput` 而不是一个字符串，图跟着第一轮走。**屏幕这一侧还没有**——
  新屏幕的命令派发压根不收图（`tui/src/commands.rs` 全文只有一处
  `images: Vec::new()`），所以这不是 `/goal` 一条的事，是「命令看不见编辑区里
  那几张图」这件更大的事，要做就整件做。

- **`/todo add|clear`** —— **不做**。这张单子是 **agent 自己的**（`TodoWrite` 写的，
  屏幕只是画它）。人改一笔，下一次 `TodoWrite` 就把它盖掉 —— 做出来是一个
  「有时候管用」的按钮，而那比没有这个按钮更坏。人要改计划，对它说就行。
