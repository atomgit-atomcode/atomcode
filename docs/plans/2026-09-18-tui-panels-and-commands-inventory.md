...
| B2-6 | `/copy`、`/save`、`/view` | 复制代码块、存 markdown、看文件浮层 |
| B2-7 | `/language` | 设置键的一种(可并进 A3) |
| B2-8 | `/whoami`、`/worktree` | 当前登录用户;worktree 隔离 |
...
| B2-10 | `/think on\|off` | 与 `/effort` 是两个旋钮:要不要思考 vs 思考多狠 |
| B2-11 | `/paste [路径]` | 兜 Windows 下 Ctrl+V 被按键层拦截、ohos 读不到剪贴板 |
