...
export interface TurnFinishedInfo {
  ...
  message?: string;        // 可选摘要
  durationMs?: number;     // 优先使用终态 stats.duration_ms；未知时按最短时长 fail-closed
  dedupeKey?: string;      // session + 终态 stats 形成的稳定终态指纹
}
...
5. 只有本 tab 主动 submit 或实际观察到 `/live state{running:true}` 后，后续终态才有
   通知资格；初始连接/重连重放的 idle state 不触发。时长优先取终态
   `stats.duration_ms`，缺失且配置了最短时长时 fail-closed；
6. 跨标签去重：以 session + stop reason + 终态 stats 形成稳定终态指纹。支持
   Web Locks 时把 localStorage 检查/写入串行化；BroadcastChannel 用作即时信号，
   旧浏览器退化为尽力去重；
...
- 权限引导：只在通知设置面板提供显式「授权浏览器通知」按钮；普通页面点击、按键和
  SSE 回调均不得触发权限请求。拒绝后通过浏览器站点设置恢复。
...
| 权限需用户手势 | 中 | 仅设置面板显式授权按钮请求；绝不在普通交互或 SSE 回调里 requestPermission |
| 多标签页重复弹 | 中 | 稳定终态指纹 + Web Locks 串行检查写入 + BroadcastChannel 信号 |
| 同一会话连续多回合重复弹 | 低 | 不同终态指纹分别通知，`tag=sessionId` 只负责系统通知视觉覆盖 |
