...
> **🔴 2026-09-20 回代码逐条核过，这张表有四处不准**，核到的实情与证据在
> [`docs/plans/2026-09-19-remaining-gaps.md`](plans/2026-09-19-remaining-gaps.md)
> 的 G 节那张表里。要点：
>
> - **第 1 条 `/config` 已经做完了**：「恢复默认」（动了契约，`ResetSetting`）与
>   「预填当前值」都落地了，按模型的动态 retry 项也补了。
> - **第 3 条 `/resume`、第 6 条 `/cd` 的「没搜索」是错的**：两者都是 `Picker`，
>   而 `Picker` 的过滤同时匹配 label 与 about，打字就在筛。真缺的分别是
>   预览 / 删除、和书签。
> - **第 5 条 `/model` 记多了一半**：每行已经带 `about` 和「当前这个」的标记，
>   真缺的是分组。
>
> 这类错法的代价是让人去重建一个已经有的东西，所以动手前先照那张表核一遍。
