...
test('shouldNotify: late join (durationMs undefined) skips duration filter', () => {
  assert.equal(
    shouldNotify({
      supported: true,
      permission: 'granted',
...
test('shouldNotify: same session within 5s dedup window → false', () => {
  assert.equal(
    shouldNotify({
      ...
      lastNotify: { sessionId: 's1', ts: 900 }, // now=1000, 差 100ms < 5000
      now: 1000,
    }),
    false,
  );
...
test('shouldNotify: same session beyond 5s window → true', () => {
  assert.equal(
    shouldNotify({
      ...
      hidden: true,
      info: makeInfo({ sessionId: 's1' }),
      lastNotify: { sessionId: 's1', ts: 1000 - 6000 },
      now: 1000,
    }),
    true,
  );
...
test('shouldNotify: different session ignores last notify → true', () => {
  assert.equal(
    shouldNotify({
      ...
      lastNotify: { sessionId: 's1', ts: 900 },
      now: 1000,
    }),
    true,
  );
